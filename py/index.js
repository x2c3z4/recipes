...
module.exports = exports = function(webot){
  ...
  function do_help(info){
      var reply = {
        title: 'Welecom to receive WIFI password',
        pic: 'http://ww3.sinaimg.cn/large/67dee169gw1esbn4sy7igj20zk0zk784.jpg',
        url: 'http://www.weibo.com/1742659945',
        description: [
            '每天推送JAPAC的密码',
            '',
            '回复a获取JAPAC密码',
 ...
 function  do_get_pass(info){
    log('get a passwd: %s', info.text);
    info.flag = true;
    var text = fs.readFileSync('/tmp/pass.txt','utf8');
    var pass = text.split('\n')[7];
    return pass.substring(pass.indexOf(" ")+1);
  }


  webot.set(/^a$/i, do_get_pass);

  webot.set(/^b$/i, function(info){
    log('get a passwd: %s', info.text);
    info.flag = true;
    var text = fs.readFileSync('/tmp/pass.txt','utf8');
    var pass = text.split('\n')[15];
