...
//	revised on: 04 MAY 2006 -- so long lines will trigger warning
//--------------------------------------------------------------------
...
int main( int argc, char *argv[] )
{
	...
	while ( fgets( in_line, sizeof( in_line), input ) )
		...
		if ( j >= CHARS_PER_LINE ) ++truncated_lines;
	...
	if ( truncated_lines ) 
		{
		printf( "\n  WARNING: some lines exceeded the paper-width: " );
		printf( "printing is cancelled.\n\n" );
		unlink( tempname ); exit(1);
		} 
